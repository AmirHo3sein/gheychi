...
export class BookingExpiryJob {
  ...
  async run(): Promise<number> {
    ...
    // that consumed a coupon code has to give that code back (see
    // releaseCouponRedemption), and that needs the exact set of bookings this run
    // expired -- atomically with the expiry itself, so a crash between the two can't
    // leave a dead booking still holding its customer's code hostage.
    return this.bookings.manager.transaction(async (em) => {
      ...
      await releaseCouponRedemption(em, expiredIds);
