<!-- apps/user-app/app/components/ui/JalaliDatePicker.vue -->
<!-- A small hand-built Shamsi (Jalali) calendar picker backed by jalaali-js for the date
     ...
     Mirrors provider-panel/admin-panel's component of the same name -- kept as this app's
     own copy per this repo's cross-app isolation convention. -->
...
const selectedJalali = computed(() => {
  ...
  const [gy, gm, gd] = props.modelValue.split('-').map(Number) as [number, number, number]
...
// The popover is 16rem wide and anchored at its trigger's inline-start edge, growing
// toward the inline-END -- which in RTL is to the LEFT. On a narrow viewport that escapes
// off the left edge, and the calendar grid ends up half unreachable. Measure the rendered
// box once it's open and translate it back inside; wherever there is room the shift is 0.
...
<template>
  <div ref="root" class="relative" @keydown="onKeydown">
    <button
      ...
      @click="toggle"
    >
      <BaseIcon name="calendar" :size="15" class="shrink-0 text-(--color-text-muted)" />
    ...
    >
      <div class="mb-2 flex items-center justify-between">
        <button type="button" aria-label="ماه بعد" class="rounded-lg p-1.5 hover:bg-(--color-surface-subtle)" @click="nextMonth">
          <BaseIcon name="chevron-forward" :size="16" class="text-(--color-text-muted)" />
        </button>
        <p class="text-sm font-bold text-(--color-text)">{{ MONTHS[viewMonth - 1] }} {{ toPersianDigits(viewYear) }}</p>
        <button type="button" aria-label="ماه قبل" class="rounded-lg p-1.5 hover:bg-(--color-surface-subtle)" @click="prevMonth">
          <BaseIcon name="chevron-back" :size="16" class="text-(--color-text-muted)" />
        </button>
      </div>
      ...
      <div class="grid grid-cols-7 gap-1 text-center">
        ...
        <button
          ...
          :class="[
            ...
            day.isSelected ? 'bg-(--color-accent) font-bold text-(--color-fill-text)' : 'hover:bg-(--color-surface-subtle)',
      ...
      <div class="mt-2 flex justify-between border-t border-(--color-border) pt-2">
