...
describe('stripUrlQuery', () => {
  it('drops the query string and fragment but keeps the path', () => {
    expect(stripUrlQuery('https://panel.gheychi.co/login?phone=09121234567')).toBe(
      'https://panel.gheychi.co/login',
    )
    expect(stripUrlQuery('https://panel.gheychi.co/bookings#token=abc')).toBe(
      'https://panel.gheychi.co/bookings',
    )
    expect(stripUrlQuery('https://panel.gheychi.co/bookings')).toBe('https://panel.gheychi.co/bookings')
  })
})
...
describe('scrubEvent', () => {
  it('removes cookies, headers, body and query string, and never sends a user', () => {
    const scrubbed = scrubEvent({
      type: undefined,
      request: {
        url: 'https://panel.gheychi.co/login?phone=09121234567',
        cookies: { session: 'jwt' },
    ...
    expect(scrubbed.request).toEqual({ url: 'https://panel.gheychi.co/login' })
