<!-- apps/admin-panel/src/components/ui/AppMoneyInput.vue -->
...
function onInput(event: Event) {
  ...
  // Persian-digit amount instead of reading it. A pasted "3,000,000" or a stray non-digit
  // keystroke both still collapse to the clean digit string this field's v-model promises.
  //
  // A leading "-" survives on purpose: this field doesn't decide whether negative amounts are
  // valid (e.g. a wallet debit adjustment) -- the caller's own Number(...) validation does.
  // Silently dropping the sign would turn an invalid/negative entry into a quietly-accepted
  // positive one instead of letting that check reject it as intended.
...
<template>
  <!-- @update:model-value is a required no-op, not decoration: without SOME listener here,
       ...
       reset (a rejected amount restoring the old value, a parent clearing the field after
