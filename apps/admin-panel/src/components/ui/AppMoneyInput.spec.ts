...
describe('AppMoneyInput', () => {
  ...
  // silently stops reflecting external resets pushed down through :model-value. A real v-model
  // consumer (a genuine 'onUpdate:modelValue' listener that writes the prop back, same shape
  // as every actual call site in this codebase) is required to exercise this -- a bare
  ...
  it('passes through label, error, and disabled to the underlying field', () => {
    const wrapper = mount(AppMoneyInput, {
      props: { modelValue: '1000', label: 'مبلغ (تومان)', error: 'مبلغ نامعتبر است', disabled: true },
    })
    expect(wrapper.text()).toContain('مبلغ (تومان)')
    expect(wrapper.text()).toContain('مبلغ نامعتبر است')
