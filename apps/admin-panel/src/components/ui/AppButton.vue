...
<template>
  <button
    ...
    :class="[
      ...
      variant === 'primary' && 'bg-(--color-accent) text-white shadow-(--shadow-sm) hover:bg-(--color-accent-strong) hover:shadow-(--shadow-md)',
      variant === 'secondary' && 'bg-(--color-border-soft) text-(--color-text) hover:bg-(--color-border)',
  ...
  >
    <AppIcon v-if="loading" name="spinner" :size="size === 'lg' ? 20 : 16" class="animate-spin" />
