...
| Worker added to a salon roster | `salons/salon-workers.controller.ts` `notifyWorkerAdded` | "you were added as staff by «salon», log in with this number" + login URL — **only if the salon is `approved`** (the roster row is created regardless). Any self-registered user can create a `pending` salon with an arbitrary name, so without the gate this was a channel for attacker-worded SMS to any number at platform cost, outside the SMS quota |
