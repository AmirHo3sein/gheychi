...
- **Prometheus metrics** (`GET /api/metrics`, `metrics/metrics.service.ts`): 22 counters/histograms — `http_requests_total`/`http_request_duration_seconds`, and per-domain outcome counters for booking (`booking_attempts/successes/failures/cancellations/completions/no_shows_total`), payment (`payment_attempts/successes/failures/refunds_total`), coupon (`coupon_validation_attempts/redemptions/rejections_total`), wallet, referral, cron (`cron_job_runs_total`, `cron_job_duration_seconds` — every job funnels through `CronJobRunner`, the single most valuable wiring point), CSP violations, and backup reports. Owns its own `prom-client` `Registry` (not the global default one) so parallel test modules never collide on "metric already registered." Every observation goes through a `safe()` wrapper — a metrics bug must never be able to fail the real request it's instrumenting. The endpoint itself is `@Public()` by necessity (a Prometheus scraper carries no session cookie) but blocked at the Caddy edge in production (`handle /api/metrics* { respond 404 }`) since it exposes real booking/payment/revenue counters; `docker/prometheus/prometheus.yml` scrapes it directly over the compose network, never through Caddy.
- **OpenTelemetry tracing** (`tracing.ts`, required as the literal first line of `main.ts`, before any other import — OTel's own required order for auto-instrumentation to patch a module before something else requires it first): instruments incoming HTTP, Postgres (`instrumentation-pg` — what TypeORM's own queries show up as, since TypeORM has no OTel instrumentation of its own), and Redis. Exporter is OTel's own `ConsoleSpanExporter` (spans as JSON to stdout) — no real tracing backend (Jaeger/Honeycomb/Datadog) is wired in this environment, same "real capability, honest default provider" shape as error-tracking/analytics; swapping in a real backend later is a one-line `traceExporter` change plus the (deliberately not-yet-installed) `@opentelemetry/exporter-trace-otlp-http` package. Known gap: external-provider calls (Zarinpal, Kavenegar, web-push) go through the global `fetch()`, not the legacy `http` module `instrumentation-http` patches, so they currently produce no span (`@opentelemetry/instrumentation-undici` would close this, left out to keep today's instrumentation footprint to exactly HTTP/pg/ioredis). `tracing-query-redaction.util.ts` strips SQL parameter values from captured spans.
